...
/**
 * The type Manager create composed pack c.
 * @author Sofía C.L.
 * @version 1.0
 */
public class ManagerCreateComposedPackC implements Controller {

    private final List<StoreProduct> products = new ArrayList<>();
    private final HashSet<Pack> packs = new HashSet<>();
    private final ManagerCreateComposedPackP pp;

    /*------------------------------------------------- CONSTRUCTOR --------------------------------------------------*/

    /**
     * Instantiates a new Manager create composed pack c.
     * @param pp the pp
     */
    public ManagerCreateComposedPackC(ManagerCreateComposedPackP pp) {
        this.pp = pp;
        initializeActions();
    }
    ...
    public void initializeActions() {
        pp.getNombre().addActionListener(e -> {
            ...
            if (toAdd == null) {
                JOptionPane.showMessageDialog(null, "Ese producto no se encuentra en la tienda.", "Error",
        ...
        pp.getAddPackButton().addActionListener(e -> {
            if (pp.getPackIdAdd().getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Debes insertar el ID del pack que quieres buscar.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            int id;
            try {
                ...
                JOptionPane.showMessageDialog(null, "El ID del pack debe ser un número válido.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            Pack toAdd = Store.getInstance().getPackById(id);
            if (toAdd == null) {
                JOptionPane.showMessageDialog(null, "No existe un pack con ese ID.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
        ...
        pp.getConfirmar().addActionListener(e -> {
            if (pp.getPackName().getText().isEmpty() || pp.getPrice().getText().isEmpty() ||
                pp.getPictureDirectory().getText().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Debes rellenar todos los datos.", "Error",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
