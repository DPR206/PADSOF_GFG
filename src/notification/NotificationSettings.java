	...
	public NotificationSettings(User owner) {
		...
		if(owner instanceof RegisteredClient)
			...
			for(Permission p : employee.getPerm())
				switch(p) {
					case Permission.EXCHANGE: this.changeInterest(NotificationType.EMPLOYEE_EXCHANGE, true);
											  this.changeInterest(NotificationType.EMPLOYEE_VALUATION, true); break;
					
					case Permission.ORDER: this.changeInterest(NotificationType.EMPLOYEE_ORDER, true); break;
